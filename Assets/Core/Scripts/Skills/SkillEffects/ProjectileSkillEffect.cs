...
public class ProjectileSkillEffect : SkillEffect {
    ...
    public override void OnStartClient() => SetInitialPosition();
        ...
        if (target != null && caster != null) {
            ...
            if (isServer && transform.position == goal) {
                ...
                if (target.health > 0)
                {
                    ...
                    caster.DealDamageAt(target, (caster.classType == PlayerType.Physical ? caster.p_atk : caster.m_atk) + damage, stunChance, stunTime);
                }
